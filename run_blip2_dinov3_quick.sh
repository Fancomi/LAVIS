#!/bin/bash
# Quick: 1/10数据量, 2 epoch — VisionEncoder 对比评估  [dinov3]
set -e
...
echo ""
echo "=========================================="
echo " BLIP2 + DINOV3  [quick]"
echo "=========================================="

echo ""
echo ">>> [1/3] Stage 1: Q-Former pretraining..."
python -m torch.distributed.run --nproc_per_node=$NPROC --master_port=$MASTER_PORT \
    train.py --cfg-path lavis/projects/blip2/train/pretrain_stage1_quick_dinov3.yaml
echo ">>> Stage 1 done."

S1_CKPT=$(find "$LIB_ROOT/output/BLIP2/Pretrain_stage1_quick_dinov3" -name "checkpoint_best.pth" 2>/dev/null | sort | tail -1)
if [ -z "$S1_CKPT" ]; then
...
python -m torch.distributed.run --nproc_per_node=$NPROC --master_port=$MASTER_PORT \
    train.py --cfg-path lavis/projects/blip2/train/pretrain_stage2_quick_dinov3.yaml \
    --options model.pretrained="$S1_CKPT"
echo ">>> Stage 2 done."

S2_CKPT=$(find "$LIB_ROOT/output/BLIP2/Pretrain_stage2_quick_dinov3" -name "checkpoint_best.pth" 2>/dev/null | sort | tail -1)
if [ -z "$S2_CKPT" ]; then
...
python -m torch.distributed.run --nproc_per_node=$NPROC --master_port=$MASTER_PORT \
    evaluate.py --cfg-path lavis/projects/blip2/eval/caption_coco_quick_eval_dinov3.yaml \
    --options model.pretrained="$S2_CKPT"
echo ">>> Evaluation done."

echo ""
echo "=========================================="
echo " DINOV3 quick done."
echo " S1 ckpt: $S1_CKPT"
echo " S2 ckpt: $S2_CKPT"
echo " Results: $LIB_ROOT/output/BLIP2/Eval_caption_quick_dinov3/"
echo "=========================================="
