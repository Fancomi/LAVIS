#!/bin/bash
# Smoke: Stage1(10步+val) → Stage2(10步+val) → Caption eval  [dinov3]
...
echo "=========================================="
echo " BLIP2 + DINOV3  [smoke]"
echo "=========================================="

echo ""
echo ">>> [1/3] Stage 1: 10 steps + val loss eval"
python -m torch.distributed.run --nproc_per_node=2 --master_port=$MASTER_PORT \
    train.py --cfg-path lavis/projects/blip2/train/pretrain_stage1_smoke_dinov3.yaml
echo ">>> Stage 1 done."

S1_CKPT=$(find "$LIB_ROOT/output/BLIP2/Smoke_stage1_dinov3" -name "checkpoint_best.pth" 2>/dev/null | sort | tail -1)
if [ -z "$S1_CKPT" ]; then
...
python -m torch.distributed.run --nproc_per_node=2 --master_port=$MASTER_PORT \
    train.py --cfg-path lavis/projects/blip2/train/pretrain_stage2_smoke_dinov3.yaml \
    --options model.pretrained="$S1_CKPT"
echo ">>> Stage 2 done."

S2_CKPT=$(find "$LIB_ROOT/output/BLIP2/Smoke_stage2_dinov3" -name "checkpoint_best.pth" 2>/dev/null | sort | tail -1)
if [ -z "$S2_CKPT" ]; then
...
python -m torch.distributed.run --nproc_per_node=2 --master_port=$MASTER_PORT \
    evaluate.py --cfg-path lavis/projects/blip2/eval/caption_coco_smoke_eval_dinov3.yaml \
    --options model.pretrained="$S2_CKPT"
echo ">>> Eval done."

echo ""
echo "=========================================="
echo " DINOV3 smoke PASSED"
